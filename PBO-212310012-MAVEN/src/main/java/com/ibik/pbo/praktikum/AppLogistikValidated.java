...
public class AppLogistikValidated {
		...
		do {
			...
			try {       
				...
				if(resi == pengiriman.get(0)) {
					konfirmasi = jop.showConfirmDialog(null, "No Resi = " + pengiriman.get(0) + "\nTotal Harga = " + pengiriman.get(1),"Apakah Anda Ingin Keluar ?",jop.YES_NO_OPTION);
				}else {
					jop.showMessageDialog(null, "Data Tidak Ditemukan","Error Message",jop.ERROR_MESSAGE);
				}
